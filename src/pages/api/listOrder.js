...
export default async function listOrder(req, res) {
  try {
    ...
    const url = `https://scotland-titles-app-development.myshopify.com/admin/api/2023-10/orders.json${req.query.name
        ? `?name=${req.query.name}`
        : req.query.page_info
      ...
      .then(async (response) => {
        ...
        if (req.query.name) {
          if (response.data.orders.length > 0) {
            console.log("respons found with query name", response.data.orders.length);
            result = { data: response.data.orders, headers: response.headers };
          }
          else {
            console.log("Empty respons with query name", response.data.orders.length);
            console.log("Proceeding with GraphQL request:",);
            const gqlQuery = `
            query {
              orders(first: 10, ${req.query.name ? `query: "name:${req.query.name}"` : 'query: "status:archived"'}) {
                  nodes {
                    id
                    name
                    createdAt
            ...
            const graphqlUrl = "https://scotland-titles-app-development.myshopify.com/admin/api/2023-10/graphql.json"
            await axios.post(graphqlUrl, { query: gqlQuery }, {
              headers: {
              ...
              .then((graphqlResponse) => {
                ...
                let updatedResponse = graphqlResponse.data.data.orders.nodes.map((item) => {
                  return {
                    id: item.id,
                    name: item.name,
                ...
                graphqlResponse?.data?.errors?.length > 0
                  ? result = { errors: graphqlResponse.data.errors, headers: graphqlResponse.headers }
                  : result = { data: updatedResponse, headers: graphqlResponse.headers }
              })
              .catch((graphqlError) => {
                console.error('GraphQL request error>>>>', graphqlError.data.errors);
